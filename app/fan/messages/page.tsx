...
export default function FanMessagesPage() {
  ...
  useEffect(() => {
    ...
    if (conversationId && conversations.length > 0) {
      ...
      if (conv) {
        ...
        router.replace('/fan/messages', undefined)
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const fetchConversations = async () => {
    ...
    try {
      const response = await fetch(`/api/messages/conversations?userId=${userId}&userType=fan`)
  ...
  const otherUser = selectedConversation?.creator
